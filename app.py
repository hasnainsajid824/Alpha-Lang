...
class CodeAnalyzerApp:
    def __init__(self, root):
        ...
        # Heading for code editor
        self.code_editor_label = ttk.Label(mainframe, text="Code Here ...", font=('Arial', 14))
        self.code_editor_label.place(x=10, y=10)
       ...
       # Run button with image
        ...
        image = image.resize((25, 25))
        self.run_button_image = ImageTk.PhotoImage(image)
        button_width = 15  # Width for all buttons

        self.run_button = ttk.Button(mainframe, text="Run", image=self.run_button_image, compound=tk.RIGHT, command=self.run_analysis)
        self.run_button.place(x=550, y=5)

        # Heading for results display
        self.results_display_label = ttk.Label(mainframe, text="Output", font=('Arial', 12))
        self.results_display_label.place(x=10, y=380)

        # Text widget for results
        self.results_display = scrolledtext.ScrolledText(mainframe, wrap=tk.WORD, width=80, height=10, background='#464646', foreground='#d3d3d3', insertbackground='white')
        self.results_display.place(x=10, y=410)
        ...
        self.assembly_button.place(x=700, y=150)

        # Show Tokens button
        self.tokens_button = ttk.Button(mainframe, text="Show Tokens", command=self.show_tokens, width=button_width)
        self.tokens_button.place(x=700, y=180)

        # Show Symbol Table button
        self.symbol_table_button = ttk.Button(mainframe, text="Show Symbol Table", command=self.show_symbol_table, width=button_width)
        self.symbol_table_button.place(x=700, y=210)

        # Clear Fields button
        self.clear_button = ttk.Button(mainframe, text="Clear Fields", command=self.clear_fields, width=button_width)
        self.clear_button.place(x=700, y=240)
